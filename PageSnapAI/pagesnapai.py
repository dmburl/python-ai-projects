#!/usr/bin/env python3
"""
Document to Markdown using Google Gemini API
With unified GUI form for batch processing.

Supports: Images (PNG, JPG, PDF, GIF, WebP), Text (TXT), Documents (DOCX, DOC)
...
def extract_text_from_file(file_path: str, max_size_mb: int = 100) -> str:
    """Extract text from documents (.txt, .docx).
    ...
    max_size = max_size_mb * 1024 * 1024
    if file_size > max_size:
        raise ValueError(f"File exceeds maximum size ({max_size_mb} MB): {path.name}")
    if file_size == 0:
    ...
    if ext == ".txt":
        try:
            ...
            raise ValueError(f"Could not decode text file (encoding issue): {path.name}")
        except Exception as e:
    ...
    # DOCX (Office Open XML)
        ...
        try:
            ...
            raise ValueError(
                "python-docx package required for .docx support. "
        ...
        try:
            ...
            if not text.strip():
                raise ValueError(f"DOCX file contains no readable text: {path.name}")
            return text
        except Exception as e:
...
def transcribe_image(file_path: str, api_key: str, model: str = "gemini-2.5-flash", max_size_mb: int = 100, prompt: str | None = None, extract_text: bool = True) -> tuple[str, float]:
    ...
    Supports:
    - Images: PNG, JPG, PDF, GIF, WebP
    - Documents: TXT, DOCX
        - extract_text=True: Extract text first, send as text (cheaper, faster)
        - extract_text=False: Send as binary to LLM (may preserve formatting better)
    ...
    prompt_text = sanitize_prompt(prompt) if prompt else "Transcribe this document to Markdown"
    if not prompt_text:
        prompt_text = "Transcribe this document to Markdown"
    ...
    # Handle image files
    if file_ext in [".png", ".jpg", ".jpeg", ".pdf", ".webp", ".gif"]:
    ...
    # Handle text/document files
        ...
        # Determine handling mode: extract text (cheaper) or send as binary (better formatting)
        if extract_text:
            # Extract text first (cheaper token usage)
            text_content = extract_text_from_file(file_path, max_size_mb)
            content_part = text_content
        else:
            # Send document as binary to LLM
            with open(file_path, "rb") as f:
                file_data = f.read()
            ...
            content_part = {"mime_type": mime_type, "data": file_data}
...
class OCRApp:
    ...
    def create_widgets(self):
        ...
        # Title
        title = ttk.Label(main_frame, text="📄 PageSnapAI", font=("Helvetica", 16, "bold"))
        ...
        # Gemini Model
        model_frame = ttk.LabelFrame(top_settings_frame, text="Gemini Model", padding="10")
        ...
        api_notice.grid(row=2, column=0, sticky="ew", pady=(0, 10))
        ...
        prompt_entry.grid(row=0, column=0, sticky="ew", padx=(0, 6))
        prompt_entry.insert(1.0, self.custom_prompt_var.get())
        
        # Sync text widget content with the StringVar
        def _sync_prompt(*args):
            self.custom_prompt_var.set(prompt_entry.get(1.0, tk.END).strip())
        
        prompt_entry.bind("<KeyRelease>", _sync_prompt)

        def _reset_prompt():
            prompt_entry.delete(1.0, tk.END)
            prompt_entry.insert(1.0, "Transcribe this image to Markdown")
        ...
        reset_btn.grid(row=0, column=1, padx=(0, 0), sticky="n")
        ...
        # Input Files Section with Document Handling checkbox
        ...
        select_btn = ttk.Button(btn_frame, text="Select Files...", command=self.select_files)
        ...
        # Document handling checkbox on the right
        doc_mode_check = ttk.Checkbutton(
            btn_frame,
            text="Extract text from documents (TXT, DOCX)",
            variable=self.extract_document_text
        )
        doc_mode_check.pack(side="right", padx=(0, 0))
        ...
        output_entry = tk.Entry(output_frame, width=60, relief=tk.SUNKEN, borderwidth=2)
    ...
    def select_files(self):
        files = filedialog.askopenfilenames(
            title="Select Images or Documents to Process",
            filetypes=[
                ("Supported files", "*.png *.jpg *.jpeg *.pdf *.webp *.gif *.txt *.docx"),
                ("Images", "*.png *.jpg *.jpeg *.pdf *.webp *.gif"),
                ("Documents", "*.txt *.docx"),
                ("PNG files", "*.png"),
                ("JPEG files", "*.jpg *.jpeg"),
                ("PDF files", "*.pdf"),
    ...
    def show_about(self):
        ...
        title = ttk.Label(frame, text="PageSnapAI", font=("Helvetica", 14, "bold"))
        title.pack(anchor="w")
